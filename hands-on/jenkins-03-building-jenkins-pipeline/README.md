...
- Or launch and configure a Jenkins Server on Amazon Linux 2 AMI with security group allowing SSH (port 22) and HTTP (ports 80, 8080) connections using the [Cloudformation Template for Jenkins on Docker Installation](./jenkins-on-docker-cfn-template.yml).
  ...
  eğer jenkinse giremezsen hazır template ile buradan da girebilirsin
...
git clone https://github.com/springbluebay/jenkins-first-project.git
- Write a simple python code which prints `Hello World` and save it as `hello-world.py`. in ec2 instance
...
- Copy the Jenkins URL from the AWS Management Console, paste it into `Payload URL` field, add `/github-webhook/` at the end of URL, and click on `Add webhook`.
  ...
  #we take the jenkins url from stack output "http://ec2-54-197-109-18.compute-1.amazonaws.com:8080/github-webhook/"

- Change the python code to print `Hello World for Jenkins Job` and save.
  ec2 üzerindeki python dosyasını değişiklik yap
...
- Create a `Jenkinsfile` (J case sensitive upper olmalı) within the `jenkins-first-project` repo and save following pipeline script.
...
- Update the `Jenkinsfile` in  ec2 with the following pipeline script, and explain the changes.
...
- To build the `java` maven project with Jenkins pipeline using the `Jenkinsfile` and `GitHub Webhook`, we will leverage from the same job created in ***Part 5*** (named as `pipeline-with-jenkinsfile-and-webhook`). To accomplish this task, we need;
