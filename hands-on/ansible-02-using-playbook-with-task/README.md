...
- Edit your yaml file (playbook2.yml) as below. First, investigate and explain the copy module with the following command.
...
ansible-playbook playbook2.yml
```
- Explain that the ``` mode``` property adds or removes permission from a user, a group, or others.
...
- Edit playbook3.yml and remove the Apache server from the hosts.
...
- This time, install Apache and wget together with playbook4.yml. After the installation, enter the IP-address of node2 to the browser and show the Apache server. Then, connect node1 with SSH and check if "wget and apache server" are running. 
...
- Remove Apache and wget from the hosts with playbook5.yml.
...
- Using ansible loop and conditional, create users with playbook6.yml. After the creation, check the servers if users are created. 
